...
func assembleTx(hash *chainhash.Hash, value int64, idx uint32, sender, receiver cashutil.Address,
	...
	script, err := txscript.NewScriptBuilder().AddData(wif.SerializePubKey()).AddOp(txscript.OP_CHECKDATASIGVERIFY).
		AddOp(txscript.OP_DUP).AddOp(txscript.OP_HASH160).
		AddData(sender.ScriptAddress()).AddOp(txscript.OP_EQUALVERIFY).AddOp(txscript.OP_CHECKSIG).
		Script()
	if err != nil {
		return nil, err
	}
